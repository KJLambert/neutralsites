        ...
        for feature in rec.features:
            if feature.type == "CDS":  # ideally regions would account for RNA features and avoid them
                try:
                    ...
                    print(
                        f"unconverted locus tag, usually on small contigs, avoid anyway: {feature.qualifiers.get('locus_tag', ['Unknown'])[0]}"
                    )
            ...
            if current_feat["strand"] == 1 and next_feat["strand"] == -1:
                ...
                if 200 <= intergenic_length <= 500:
                    region = {
                        ...
                        "upstream_gene_annotation": "gene: "
                        + current_feat["gene"]
                        + " product: "
                        + current_feat["product"],
                        "downstream_gene_annotation": "gene: "
                        + next_feat["gene"]
                        + " product: "
                        + next_feat["product"],
                        ...
                        "flanking_sequence": rec.seq[
                            max(0, current_feat["end"] - 1000) : min(len(rec.seq), next_feat["start"] + 1000)
                        ],
                    }
